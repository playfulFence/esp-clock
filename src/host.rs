...
impl EmbeddedGraphicsFrameBuffer {
    ...
    pub(crate) fn led_draw<D>(&self, 
        display: &mut D,
        writable: &String, 
        color_conv: fn(ZXColor, ZXBrightness) -> D::Color) -> Result<(), D::Error>
    where
        D: DrawTarget + Dimensions,
        //D::Color: From<Rgb565>,
    {
        //display.clear(color_conv(ZXColor::Black, ZXBrightness::Normal))?;
    
        Rectangle::new(display.bounding_box().top_left, display.bounding_box().size)
            .into_styled(
                PrimitiveStyleBuilder::new()
                    .fill_color(color_conv(ZXColor::Blue, ZXBrightness::Normal))
                    .stroke_color(color_conv(ZXColor::Yellow, ZXBrightness::Normal))
                    .stroke_width(1)
                    .build(),
            )
            .draw(display)?;
        ...
        Text::new(
            ...
            MonoTextStyle::new(&FONT_10X20, color_conv(ZXColor::White, ZXBrightness::Normal)),
        )
        .draw(display)?;
